...
def merge_images_as_long_image(images_path: str, images_amount: int, long_image_path: str) -> bool:
    try:
        ...
        with tqdm(total=images_amount) as pbar:
            for image_id in range(images_amount):
                tmp_image = Image.open(images_path + "/" + "images_%s.png" % image_id)
                if long_image is None:
                    each_tmp_image_size = tmp_image.size
                    long_image = Image.new("RGB", (each_tmp_image_size[0], images_amount * each_tmp_image_size[1]), (250,250,250))
...
def convert_pdf_to_long_image(
    ...
    if images_path is None: images_path = gen_random_tmp_path()
    if long_image_path is None: long_image_path = pdf_path.replace(".PDF", ".jpg").replace(".pdf", ".jpg")
    images_amount = convert_pdf_to_images(pdf_path, images_path)
    assert images_amount > 0
    assert merge_images_as_long_image(images_path, images_amount, long_image_path)
    assert clean_tmp_images(images_path)


if "__main__" == __name__:
    parser = argparse.ArgumentParser(description='Convert PDF to Long Image.')
    parser.add_argument('--pdf_path', help='the path of PDF wanna to convert', type=str, required=True)
    parser.add_argument('--long_image_path', help='the PDF convert result long image path', type=str, required=False, default=None)
    parser.add_argument('--tmp_images_path', help='the images path before converted to long image', type=str, required=False, default=None)

    args = parser.parse_args()
    convert_pdf_to_long_image(args.pdf_path, args.long_image_path, args.tmp_images_path)
